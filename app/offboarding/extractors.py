...
from asana.utils import get_date_field_value_from_task, get_field_value_from_task
...
def extract_offboarding_task_data(task_data: dict[str, Any]) -> TaskData:
    ...
    fired_date = get_date_field_value_from_task(field_name="Дата увольнения", task_data=task_data)
    url = task_data["permalink_url"]
    if not all([fio, tag, position, fired_date]):
        missing_fields = [
            name
            for name, value in (("fio", fio), ("tag", tag), ("position", position), ("fired_date", fired_date))
            if not value
        ]
    ...
    assert fired_date is not None  #  noqa: S101
    return TaskData(fio=fio, position=position, tag=tag, url=url, fired_date=fired_date)
